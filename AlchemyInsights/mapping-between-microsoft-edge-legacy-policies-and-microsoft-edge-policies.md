---
title: Teave Microsoft Edge ' i Pärandipoliitika ja Microsoft Edge ' i poliitikate vahel vastendamise kohta
ms.author: v-smandalika
author: v-smandalika
manager: dansimp
ms.date: 03/18/2021
audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Priority
ms.collection: Adm_O365
ms.custom:
- "8218"
- "9004596"
ms.openlocfilehash: 1d7a2e36c104a1e1238a4c6d5516b04b7c4de46a
ms.sourcegitcommit: c08bed4071baa3bb5879496df3ed44fb828c8367
ms.translationtype: MT
ms.contentlocale: et-EE
ms.lasthandoff: 03/19/2021
ms.locfileid: "51035267"
---
# <a name="learn-about--the-mapping-between-microsoft-edge-legacy-policies-and-microsoft-edge-policies"></a>Teave Microsoft Edge ' i Pärandipoliitika ja Microsoft Edge ' i poliitikate vahel vastendamise kohta

Microsoft Edge ' i pärand (versioonid 45 ja varasem versioon) ning mobiilsideseadmete halduse (MDM) sätted vastendatakse Microsoft Edge ' i versiooni 80 vastava poliitikaga. Lisateavet leiate teemast [Microsoft Edge ' i pärand Microsoft Edge ' i poliitika vastendamine](https://docs.microsoft.com/deployedge/microsoft-edge-policy-map-legacy-to-newedge).