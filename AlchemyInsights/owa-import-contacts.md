---
title: '8000019: kuidas lisada või importida kontakte'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 729c974995f4f4da3c2370dc7804416035dda508
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: et-EE
ms.lasthandoff: 04/22/2020
ms.locfileid: "43742909"
---
# <a name="import-contacts"></a>Kontaktide importimine

Enne kontaktide importimist Outlook Web, peate need eksportida oma teise e-posti pakkuja csv-failina. Seejärel järgige alltoodud juhiseid.

1. Valige lehe allosas <img src='data:image/png;base64,' />
 **Inimesi**.
2. Tehke tööriistaribal valik **Halda** > **impordi kontakte**.
3. Valige **Sirvi**, valige oma CSV-fail ja seejärel valige **Ava**.
4. Valige **impordi**.

Lisateabe saamiseks vaadake [Outlooki kontaktide importimine](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

