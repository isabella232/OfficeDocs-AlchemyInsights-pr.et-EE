---
title: Testimine Alchemy tekstisiseseid pilte
ms.author: pebaum
author: pebaum
ms.date: 04/21/2020
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 47e4d86125ae73faa97a777ef1c56620bd0b890b
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: et-EE
ms.lasthandoff: 04/22/2020
ms.locfileid: "43759976"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="d3883-102">Juurdepääs domeenide lehele</span><span class="sxs-lookup"><span data-stu-id="d3883-102">Access the Domains Page</span></span>

<span data-ttu-id="d3883-103">Domeenide lehele juurdepääsuks klõpsake seda ikooni allpool:</span><span class="sxs-lookup"><span data-stu-id="d3883-103">To access the domains page, just click this icon below:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="d3883-104">Seejärel saate oma domeeni lisada, muuta või eemaldada.</span><span class="sxs-lookup"><span data-stu-id="d3883-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="d3883-105">Lisateabe saamiseks vaadake [DOMEENIDE KKK.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span><span class="sxs-lookup"><span data-stu-id="d3883-105">For more information, see the [Domains FAQ.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span></span>
  